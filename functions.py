...
def parse_arguments(credentials_file):
    try:
        parser = argparse.ArgumentParser(
            description="Automating the process of adding new challenges to the database."
        )
        ...
        parser.add_argument(
            ...
            "--link",
            type=str,
            help="Specify the challenge link/url",
            required=True,
        )
        parser.add_argument(
            "-u",
            "--username",
            type=str,
            default=parse_credentials(credentials_file),
    ...
    match platform:
        case "Codewars":
            ...
            for x in range(Pages):
                current_page = "https://www.codewars.com/api/v1/users/{}/code-challenges/completed?page={}".format(
                    username, x
                )
                ...
                for challenge in response_json["data"]:
                    challenge_data[str(challenge["id"]).strip()] = {
                        ...
                        "Completed at": translate_date(
                            challenge["completedAt"].strip()
                        ),
                        "Additional Info": {},
                    }
                ...
                "Statistics": {
                    "1-kyu": len(
                        [
                            ...
                            if v["Additional Info"]["rank"] == "1-kyu"
                        ]
                    ),
                    "2-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "2-kyu"
                        ]
                    ),
                    "3-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "3-kyu"
                        ]
                    ),
                    "4-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "4-kyu"
                        ]
                    ),
                    "5-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "5-kyu"
                        ]
                    ),
                    "6-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "6-kyu"
                        ]
                    ),
                    "7-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "7-kyu"
                        ]
                    ),
                    "8-kyu": len(
                        [
                            k
                            for k, v in challenge_data.items()
                            if v["Additional Info"]["rank"] == "8-kyu"
                        ]
                    ),
                },
...
def edit_template(
    ...
    try:
        ...
        template_content = template_content.replace(
            ...
            f'<a id="challenge_name" href="{challenge_url}">{challenge_name}</a>',
        )
        template_content = template_content.replace(
            '<i id="completion_date" align="center"></i>',
            f'<i id="completion_date" align="center">{completion_date}</i>',
        )
        template_content = template_content.replace(
            '<i id="tags" align="center"></i>',
            f'<i id="tags" align="center">{tags}</i>',
        )
